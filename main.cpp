...
void drawWorkingNonogramMatrix(ImDrawList* drawList, nonogram* RandoNono, float horOffset, float verOffset, float horWidth, float verWidth);
...
int main(){
    ...
    while (!glfwWindowShouldClose(window)){
        ...
        if(!*renderText){
            ...
            if(!*seeSolution){
                drawWorkingNonogramMatrix(drawList, RandoNono, horWidth, verWidth, horWidth, verWidth);
            }else{
            ...
            if(!*seeSolution){
                ...
                drawWorkingNonogramMatrix(drawList, RandoNono, horOffsetTxt, verOffsetTxt, horWidth, verWidth);
            } else{
                drawSolutionNonogramMatrix(drawList, RandoNono, horOffsetTxt, verOffsetTxt, horWidth, verWidth);
            }
        }
        
        //MAIN LOGIC RUNNING IN LOOP 
        //RandoNono->mainLogicFuux();
...
void drawWorkingNonogramMatrix(ImDrawList* drawList, nonogram* RandoNono, float horOffset, float verOffset, float horWidth, float verWidth){
    for(uintmax_t i=0; i < RandoNono->rows; i++){
        for(uintmax_t j =0; j < RandoNono->cols; j++){
            ...
            if(RandoNono->nonoWorking[i][2*j]) { 
            ...
            if(RandoNono->nonoWorking[i][(2*j) + 1]) { 
                drawList->AddRectFilled(
                    ImVec2( // p_min so bottom left (x1, y1) 
                        horOffset + ((j) * horWidth), 
                        verOffset + ( (i) * verWidth)
